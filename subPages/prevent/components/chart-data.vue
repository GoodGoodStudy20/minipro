		...
		computed:{
			recordOrderP(){
				...
				this.list.forEach(item=>{
					// console.log('cc',item.recordOrder);
					arr.push(item.recordOrder)
				})
				if(!arr.length) return 0
				let temp = arr.reduce((prev,cur)=> prev+cur)
				return temp ? temp:0
			},
			successOrder(){
				let arr = []
				this.list.forEach(item=>{
					// console.log('cc',item.recordOrder);
					arr.push(item.sucessOrder)
				})
				if(!arr.length) return 0
				let temp = arr.reduce((prev,cur)=> prev+cur)
				// console.log('ctemp',temp);
				return temp
			}
		},
		components: {
		...
		methods: {
			...
			async seaterPieFun() {
				try {
					...
					// 订单总量
					this.allOrderNum = Number(res.sucessOrder) + Number(res.progressOrder) + Number(res
							.cancelOrder) +
						Number(res.rebackOrder) + Number(res.failureOrder)
					// 已完成
					...
					this.chartPieList = {
						series: [{
							data: [{
								...
								"labelText": `已完成 ${res.sucessOrder}人`
							}, {
								"name": "进行中",
								"value": res.progressOrder,
								"labelText": `进行中 ${res.progressOrder}人`
							}, {
								"name": "已取消",
								...
								"labelText": `已退回 ${res.rebackOrder}人`
							}, {
								"name": "已失败",
								"value": res.failureOrder,
								"labelText": `已失败 ${res.failureOrder}人`
							}]
						}]
					};
				} catch (e) {
