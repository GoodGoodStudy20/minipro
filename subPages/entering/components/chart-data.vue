<template>
    ...
    <view class="city-title">
      ...
      <view class="city-item" v-if="cityRange.length!=0">
              <uni-data-select
                class="select-boxone"
                v-model="cityVal"
                :localdata="cityRange"
    ...
    <view class="">
      <Chart v-if="AreaArr.length != 0" class="chart" :chartPie="AreaChartPieList" />
      <view class="AreaArr" v-else>
      	暂无数据
      </view>
    </view>
		<view class="">
  ...
  created() {
   ...
   options: { styleIsolation: 'shared' },
     ...
     async getData(){
        ...
        console.log('rrr',res);
        this.cityRangeItme = res[0].text
        this.getArea(res[0].text,0)
        this.$nextTick(()=>{
          this.cityRange = res 
        })
        console.log('cityRange',this.cityRange);
      },
      statusChange(e){
        console.log(e);
        this.getArea(this.cityRangeItme,e)
      },
      cityChange(e){
        console.log(this.cityRange[e].text);
        this.cityRangeItme = this.cityRange[e].text
        this.getArea(this.cityRange[e].text,0)
      },
    async getArea(e,t){
      let data= {
        area:e,
      ...
      const res = await selectAreaCount(data)
      console.log('area',res[0].mapVoList);
      let Arry = []
      res[0].mapVoList.forEach(item=>{
        Arry.push({
          "name": item.key,
          "value": item.value,
          "labelText": `${item.key} ${item.value}单`
        })
      })
      this.AreaArr = Arry
      console.log('Arry',Arry);
      this.AreaChartPieList = {
      						series: [{
      							data: Arry
      						}]
      					};
			...
			gotoDetails(status) {
				...
				if (status === '录单') {
					...
					url =
						`/pages/my/seats/my-order?currenctIndex=${status}&id=0&date=${this.month}&currenctIndex2=${currenctIndex2}`
				}
				uni.navigateTo({
					url
				})
         ...
         res = await driverPie(params)
         }else{
         res = await seaterPie(params)
         }
       ...
       //        {
        ...
        let res
        if(this.switchover === '0'){
        res = await driverTreeVo(params)
        }else{
        res = await seaterTreeVo(params)
        }
