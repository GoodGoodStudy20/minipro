...
<!-- 		<view class="top" :class="[
      ...
      <view class="top" :class="[
      	item.orderType ==3 || item.orderType == 4 || item.orderType == 21? 'red' :item.orderType == 11 || item.orderType ==12||item.orderType ==13||item.orderType ==14?'primary': item.orderType == 20 ? 'green':'yellow'
      	]">
      ...
      console.log('item',this.item);
		...
		methods: {
			orderItemDetailsInfo(item) {
				uni.navigateTo({
					url: `/subPages/entering/order-item-detailsInfo?numberPlate=${item.numberPlate}&id=${item.id}&createrId=${item.createrId}`,
				})
			},
			// 取消订单
