<template>
	<view class="seats">
		<view class="">
			<Header @change="headerChange" :nameH="nameH"/>
		</view>
...
<script>
  import{mapMutations} from 'vuex'
    ...
    onLoad(e){
      this.switchover = e.index
      uni.setStorageSync('switchover', e.index)
      if(e.index==='0'){
      console.log('index',e.index);
      uni.setNavigationBarTitle({
        title:"司机管理列表"
      })
      this.nameH = ['司机管理', '数据分析']
      }
      ...
      ...mapMutations(['setDate','setStarTime','setEndTime']),
          ...
          this.setEndTime('')
			...
			async seaterListFun() {
				...
				try {
          let res
          // console.log('switchover',this.switchover);
          // console.log('params',params);
          if(this.switchover === '0'){
          res = await driverManageList(params)
          }else{
					res = await catalogueList(params)
          }
