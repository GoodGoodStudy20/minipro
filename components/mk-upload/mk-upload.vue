  ...
  <view class="mk-upload" :class="{'reverse': reverse}">
    ...
    <block v-if="imgList.length >= column && reverse">
      ...
      <block v-if="imgItemAppendLength === 1">
        ...
        <block v-if="column == 4">
          <view class="mk-upload_item" :class="uploadItemColumn" v-for="(item, index) in (imgItemAppendLength + 1)" />
        </block>
        <block v-if="column == 5">
          <view class="mk-upload_item" :class="uploadItemColumn" v-for="(item, index) in (imgItemAppendLength + 2)" />
        </block>
      </block>
    ...
    <view class="mk-upload_item" :class="uploadItemColumn" v-if="controlShow && imgList.length < maxCount">
      <view class="mk-upload_add" @tap="onChooseImage" :style="{'background-color': bgcolor}"
        style="width: 60px;height: 90px; border: 2px solid #e8e8e8;">+</view>
    </view>
    <!-- 上传按钮 -->
    <xCompress ref="xCompress" />
  </view>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      action1() {
        return BASE_URL + '/swagger-order/zjcOrder/uploadFileTest'
      },
      uploadItemColumn() {
