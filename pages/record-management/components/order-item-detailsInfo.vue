  ...
  <view class="order-item-details-box">
    ...
    <view class="progress-box">
      <view class="top">
        ...
        <text class="right error" <text class="right error"
    ...
    <view class="order-item-details">
      ...
      <view class="key-box"
        v-if="orderDeatilsInfo.orderType ==6|| orderDeatilsInfo.orderType==12 || orderDeatilsInfo.orderType==13 || orderDeatilsInfo.orderType==18">
        <text class="key">失败原因:</text>
        <!-- <text class="value error">{{ orderDeatilsInfo.failureReason  || '-' }}</text> -->
      ...
      <view class="key-box">
        ...
        <view class="yearly-inspection">
          ...
          <image @click="preivewImg(orderDeatilsInfo.annualInspectionImage)"
            :src="orderDeatilsInfo.annualInspectionImage" class="yearly-inspection-image" mode="aspectFit">
          </image>
        </view>
      </view>
    ...
    <view class="">
      <view class="vehicle-collection">
        ...
        <view class="bottom-img">
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.pickupOrderPhoto)" :src="orderDeatilsInfo.pickupOrderPhoto"
              class="img" mode="aspectFill"></image>
          </view>
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.pickupFrontPhotoTwo)" :src="orderDeatilsInfo.pickupFrontPhotoTwo"
              class="img" mode="aspectFill"></image>
          </view>
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.pickupDrivingLicensePhoto)"
              :src="orderDeatilsInfo.pickupDrivingLicensePhoto" class="img" mode="aspectFill"></image>
          </view>
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.pickupOdometerPhoto)" :src="orderDeatilsInfo.pickupOdometerPhoto"
              class="img" mode="aspectFill"></image>
          </view>
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.pickupFrontPhoto)" :src="orderDeatilsInfo.pickupFrontPhoto"
              class="img" mode="aspectFill"></image>
          </view>
        </view>
      </view>
      <view class="vehicle-collection">
        ...
        <!-- <view class="bottom-img">
					<view class="img-box" v-for="(item, index) in returnTheCarList" :key="index"
						@tap="previewImageFuns">
						<image src="" class="img" mode="widthFix"></image>
					</view>
				</view> -->
        <view class="bottom-img">
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.returnOrderPhoto)" :src="orderDeatilsInfo.returnOrderPhoto"
              class="img" mode="aspectFill"></image>
          </view>
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.returnFrontPhotoTwo)" :src="orderDeatilsInfo.returnFrontPhotoTwo"
              class="img" mode="aspectFill"></image>
          </view>
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.returnDrivingLicensePhoto)"
              :src="orderDeatilsInfo.returnDrivingLicensePhoto" class="img" mode="aspectFill">
            </image>
          </view>
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.returnOdometerPhoto)" :src="orderDeatilsInfo.returnOdometerPhoto"
              class="img" mode="aspectFill"></image>
          </view>
          <view class="img-box">
            <image @click="preivewImg(orderDeatilsInfo.returnFrontPhoto)" :src="orderDeatilsInfo.returnFrontPhoto"
              class="img" mode="aspectFill"></image>
          </view>
        </view>
      </view>
    </view>
    <view class="map" v-if="Mapshow">
      <map style="width: 100%;" :markers="markers" :polyline="polyline" :longitude="longitude"
        :latitude="latitude" :scale="scale"></map>
    </view>
    <PreviewImage ref="showRef" :urlList="''" />
    <view class="distribute"
      v-if="SuperSU && orderDeatilsInfo.orderStatus != 4 && (orderDeatilsInfo.orderType == 12 || orderDeatilsInfo.orderType == 13)">
      <button type="primary" @tap="orderItemDetailsInfo(orderDeatilsInfo)">重新派单</button>
    </view>
    <view class="distribute" v-else-if="SuperSU && orderDeatilsInfo.orderStatus == 3 && orderDeatilsInfo.orderType==3">
      <button type="primary" @tap="orderItemDetailsInfo(orderDeatilsInfo)">去派单</button>
    </view>
    <view class="distribute" v-else>
      <button type="primary" @tap="close">返回</button>
    </view>
    <!-- <view class="distribute" v-else> 111111-->
    <!-- <button v-if="orderDeatilsInfo.orderStatus ==0 || orderDeatilsInfo.orderStatus ==2" type="default" @tap="goDetailsInfo(1,orderDeatilsInfo)">已派单</button>
      <button v-if="orderDeatilsInfo.orderStatus ==6" type="default">派单完成</button>
      <button v-if="orderDeatilsInfo.orderStatus ==3" type="default"class="send-orders"
      	@tap="goDetailsInfo(3,orderDeatilsInfo)">去派单</button>
      <button v-if="orderDeatilsInfo.orderStatus ==7 || orderDeatilsInfo.orderStatus ==8" type="warn"
      	@tap="goDetailsInfo(2,orderDeatilsInfo)">重新派单</button> -->
    <!-- <button type="primary" @tap="goDetailsInfo(orderDeatilsInfo)">去派单</button> -->
    <!-- <button type="primary" @tap="close">返回</button> 111111-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        markers: [{
          id: 0,
          latitude: 22.53332, //纬度
          longitude: 113.93041, //经度
          // 　iconPath: require('../../static/images/search.svg'),    //显示的图标        
          rotate: 0, // 旋转度数
          width: 20, //宽
          height: 30, //高
          title: '开始位置', //标注点名
    ...
    onLoad(options) {
      ...
      // this.id = options.id
      // this.createrId = options.id
      // const eventChannel = this.getOpenerEventChannel();
      // eventChannel.on('acceptDataFromOpenerPage', function(data) {
      // 	this.orderDeatilsInfo = data
      // 	console.log(data)
      // 	// 0 派单成功  2 派单中  5 订单失败 6 订单完成 
    ...
    computed: {
      SuperSU() {
        console.log('user1111', uni.getStorageSync('userInfo').role);
    ...
    methods: {
      async getMAP(e) {
        const res = await getOrderTrajectory({
          orderId: e.id
        })
        if(res.length){
            console.log('e', res);
            this.markers[0].longitude = res[0].lon
            this.markers[0].latitude = res[0].lat
            this.longitude = res[0].lon
            this.latitude = res[0].lat
            res.forEach(item=>{
              this.polyline[0].points.push({latitude: item.lat,longitude: item.lon})
            })
      ...
      goDetailsInfo(type, item) {
        ...
        if (type == 1) {
          ...
          url =
            `/pages/dispatch-management/send-orders-detailsInfo?type=${type}&id=${item.id}&createrId=${item.createrId}`
        }
        uni.navigateTo({
          url
        })
      ...
      speedOfProgressFun() {
        ...
        let str = '' // 状态
        // 3.未接单,4.已接单11.联系客户中,12.转单求助,13.退回订单,14.已联系客户,15.已取车,16.到达检测站,17.检测成功,18.检测失败,19.还车中,20.已还车21.等待接单 22 取车中 23 重新派单
