<template>
	<view class="order-item" @tap="orderItemDetailsInfo(item)">
		<!-- <view class="top" :class="[
			item.orderStatus == 0 || item.orderStatus == 6 ? 'green': '', item.orderStatus == 1 || item.orderStatus == 4 || item.orderStatus == 5 || item.orderStatus == 7 || item.orderStatus == 8 || item.orderStatus == 12 ? 'yellow': '',
			item.orderStatus == 2? 'primary': '', item.orderStatus == 3 ? 'blue': '',
			]"> -->
     <view class="top" :class="[
    item.orderType ==3 || item.orderType == 4 ? 'red' :item.orderType == 11 || item.orderType ==12||item.orderType ==13||item.orderType ==14?'primary': item.orderType == 20 ? 'green':'yellow'
     	]">
			...
			<text class="label" v-if="item.orderType ==11">正在联系</text>
			<text class="label" v-if="item.orderType ==12">转单求助</text>
			<text class="label" v-if="item.orderType ==13">取车失败</text>
			<text class="label" v-if="item.orderType ==14">完成联系</text>
			<text class="label" v-if="item.orderType ==15">已取车</text>
			<text class="label" v-if="item.orderType ==16">到达站点</text>
			...
			<!-- // 0 派单成功 1 派单失败  2 派单中 3 未派单 4 取消订单 5 订单失败 6 订单完成 7 订单退回 8 取车失败 12 转单求助-->
			...
			<view class="label" v-show="item.orderStatus ==12">
				转单求助
			</view> -->
		</view>
		...
		<view class="time-box">
			<view class="time-image-box">
				...
				<text class="time">{{ item.driverName || '--' }}</text>
				<text class="time sm">{{item.driverPhone || '--'}}</text>
			</view>
		</view>
		<view class="time-box fe">
			<view class="sj-right-box" v-if="isShowrole && item.orderStatus == 7">
			...
			<view class="sj-right-box" v-if="isShowrole && item.orderStatus == 4">
      ...
      isShowrole(){
		...
		methods: {
			orderItemDetailsInfo(item) {
				uni.navigateTo({
					url: `/pages/record-management/components/order-item-detailsInfo?numberPlate=${item.numberPlate}&id=${item.id}&createrId=${item.createrId}`,
				})
			},
			// 取消订单
