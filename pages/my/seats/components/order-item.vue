    ...
    computed: {
		...
		methods: {
			...
			gotoDetails(status) {
				let url = ''
				if (status === '录单') {
					url = `/pages/record-management/record-management?type=1&seaterId=${this.item.seaterId}&time=${this.date}&startTime=${this.startTime}&endTime=${this.$store.state.endTime}` // type 1 管理员
				} else {
					url = `/pages/my/seats/my-order?currenctIndex=${status}&id=${this.item.seaterId}&time=${this.date}&startTime=${this.startTime}&endTime=${this.$store.state.endTime}`
				}
				uni.navigateTo({
					url
				})
