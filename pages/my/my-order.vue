...
<script>
	...
	import Order2 from '../record-management/components/order-item2.vue'
	export default {
		components: {
			Order2,
			Search1,
		...
		methods: {
			...
			currentIndexFun(index) {
				if(index == 3){
					this.currenctIndex2 = 0 // 处理二级导航
				}
				this.currenctIndex = index
				// 0 派单成功 1 派单失败  2 派单中 3 未派单 4 退回订单 5 订单失败 6 订单完成
				this.orderStatusFun(index)
				this.initData()
				this.getAllOrderInfoListFun()
				this.top()
			},
			...
			async getAllOrderInfoListFun() {
				...
				try {
					const res = await getAllOrderInfoList(params)
