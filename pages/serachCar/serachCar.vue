<template>
    ...
    <view class="search-car-all">
        <!-- 背景图 -->
		 <view class="bg-icon" :class="set">
		    ...
		    <view class="bg-text">
		        ...
		        <view class="bg-text-bottom">输入完整车牌号</view>
		    </view>
		</view>
        ...
        <view class="search-car-input" @click="carInputClick">
            ...
            <view class="search-car" @click.stop="addSerach">确定添加</view>
        </view>
...
<script>
    ...
    export default {
        ...
        methods: {
			...mapMutations(['setCarNumber','setnewCarNumber']),
            /**
             * @desc 车牌选择关闭和打开
             */
            showOrHide(falg) {
            ...
            addSerach() {
                ...
                if (this.plateNum.length > 6) {
					uni.setStorageSync('carNum',this.plateNum)
					// this.$store.commit('setCarNumber',this.plateNum)
					this.setCarNumber(this.plateNum)
					this.setnewCarNumber(this.plateNum)
